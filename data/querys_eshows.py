...
LEFT JOIN T_CANDIDATOS C ON C.FK_OPORTUNIDADE = O.ID
	...
	AND O.DATA_INICIO < DATE_FORMAT(NOW(), '%Y-%m-31')
...
SELECT 
   ...
   FROM (
      ...
      WHERE O2.DATA_INICIO BETWEEN '2025-01-01' AND '2025-09-20'
  ...
  (SELECT COUNT(*)
   FROM (
      ...
      LEFT JOIN T_CANDIDATOS C2 ON C2.FK_ATRACAO = A2.ID
      LEFT JOIN T_OPORTUNIDADES O2 ON O2.ID = C2.FK_OPORTUNIDADE
      WHERE O2.DATA_INICIO BETWEEN '2025-09-20' AND CURDATE()
        AND O2.FK_CONTRATANTE NOT IN ('102','196','1015','1861','1871','1982','1983','1985','2624')
      GROUP BY A2.ID
      HAVING COUNT(C2.ID) > 35
   ) AS artistas_ativos
  ) AS 'Candidatos Ativos 2',
...
FROM T_OPORTUNIDADES O
LEFT JOIN ZLOG_T_OPORTUNIDADES ZO
  ON ZO.ID = O.ID
LEFT JOIN (
  SELECT
    O2.ID AS O_ID,
  ...
  LEFT JOIN T_CANDIDATOS C2
    ON C2.FK_OPORTUNIDADE = O2.ID
  GROUP BY O2.ID
) CAN
  ON CAN.O_ID = O.ID
WHERE ZO.DATA_INICIO >= '2024-01-01'
  AND ZO.DATA_INICIO < DATE_FORMAT(NOW(), '%Y-%m-31')
  AND O.DATA_INICIO >= '2024-01-01'
  AND O.DATA_INICIO < DATE_FORMAT(NOW(), '%Y-%m-31')
